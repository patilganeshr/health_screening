...
SharpiTech.SalesBillItemRegister = (function () {
    ...
    function cacheDOM() {
        ...
        DOM.salesBillItemsRegisterList = document.getElementById('SalesBillItemsRegisterList');
    }
    ...
    var getReportURL = function() {
        ...
        if (DOM.reportByFinancialYear.checked) {
            if (DOM.financialYear.selectedIndex > 0) {
                url = "GetSalesBillItemRegisterByWorkingPeriod/" + DOM.financialYear.options[DOM.financialYear.selectedIndex].value;
            }
        }
        else if (DOM.reportByCustomer.checked) {
            if (DOM.customer.selectedIndex > 0) {
                url = "GetSalesBillItemRegisterByCustomer/" + DOM.customer.options[DOM.customer.selectedIndex].value;
            }
        }
        else if (DOM.reportByFromToDate.checked) {
            if (DOM.fromDate.value !== "" && DOM.toDate.value !== '') {
                url = "GetSalesBillItemRegisterByFromToDate"; // "/" + DOM.fromDate.value + '/' + DOM.toDate.value + '/';
            }
        }
        else if (DOM.reportByFinancialYearAndVendor.checked) {            
            if (DOM.financialYear.selectedIndex > 0 && DOM.customer.selectedIndex > 0) {
                url = "GetSalesBillItemRegisterByCustomerAndWorkingPeriodId/" + DOM.customer.options[DOM.customer.selectedIndex].value + '/' + DOM.financialYear.options[DOM.financialYear.selectedIndex].value;
            }
        }
        else if (DOM.reportByCustomerAndFromToDate.checked) {            
            if (DOM.customer.selectedIndex > 0 && (DOM.fromDate.value !== "" && DOM.toDate.value !== "")) {
                url = "GetSalesBillItemRegisterByCustomerAndFromToDate/" //+ DOM.customer.options[DOM.customer.selectedIndex].value + '/' + DOM.fromDate.value + '/' + DOM.toDate.value + '/';
            }
        }
    ...
    function generateSalesBillRegister() {
        ...
        DOM.salesBillItemsRegisterList.length = 0;

        try {

            var tableBody = DOM.salesBillItemsRegisterList.tBodies[0];
    ...
    function bindReportData() {
        ...
        if (SalesBillItemsRegister.length > 0) {
            ...
            if (SalesBillItemsRegister.length) {

                var data = "";

                for (var r = 0; r < SalesBillItemsRegister.length; r++) {

                    data = data + "<tr>";
                    data = data + "<td>" + SalesBillItemsRegister[r].SalesBillNo + "</td>";
                    data = data + "<td>" + SalesBillItemsRegister[r].SalesBillDate + "</td>";                    
                    data = data + "<td>" + SalesBillItemsRegister[r].CustomerName + "</td>";
                    data = data + "<td>" + SalesBillItemsRegister[r].LRNo + "</td>";
                    data = data + "<td>" + SalesBillItemsRegister[r].TransporterName + "</td>";
                    data = data + "<td>" + SalesBillItemsRegister[r].ItemQuality + "</td>";
                    data = data + "<td>" + SalesBillItemsRegister[r].ItemName + "</td>";
                    data = data + "<td>" + SalesBillItemsRegister[r].SaleQty + "</td>";
                    data = data + "<td>" + SalesBillItemsRegister[r].UnitCode + "</td>";
                    data = data + "<td>" + SalesBillItemsRegister[r].TaxableValue + "</td>";
                    data = data + "<td>" + SalesBillItemsRegister[r].GSTAmount + "</td>";
                    data = data + "<td>" + SalesBillItemsRegister[r].TotalItemAmount + "</td>";
                    data = data + "<td>" + SalesBillItemsRegister[r].RoundOffAmount + "</td>";
                    data = data + "</tr>";
    ...
    function fnExcelReport() {
        ...
        tab = DOM.salesBillItemsRegisterList; // id of table
...
SharpiTech.SalesBillItemRegister.init();
